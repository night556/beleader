...
BeLeader 是一个通用 AI 助手，像真正的团队一样工作。告诉它你想要什么——「帮我做一个 Todo 应用」「调研最新的 AI 趋势」或「整理桌面文件」——它会启动 Coordinator 规划任务，然后并行创建多个 Worker 读取文件、搜索、浏览网页、控制桌面。每个 Worker 有独立的工具和上下文，你实时看着整个团队协作。
...
| 首页 | Main 对话 — 创建项目 | 项目视图 — Worker 运行中 |
|:----:|:--------------------:|:----------------------:|
| ![首页](example_image/homepage.png) | ![创建项目](example_image/main-create.png) | ![项目运行](example_image/project-workers.png) |
| 刚打开的空闲状态 | 用自然语言创建项目 | Coordinator + Workers 并行执行 |
